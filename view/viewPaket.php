<?php 
                         $id = 1;
                         $datas = mysqli_query($koneksi, "SELECT tb_paket.*, tb_outlet.nama AS namaOutlet FROM tb_paket INNER JOIN tb_outlet ON tb_paket.id_outlet = tb_outlet.id");
    ...
    <div class="mx-auto max-w-screen-2xl px-4 lg:px-12">
        ...
        ?>
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 "><?= $data['namaOutlet'] ?></td>
                            <td class="px-4 py-3 "><?= $data['jenis'] ?></td>
                            <td class="px-4 py-3 "><?= $data['nama_paket'] ?></td>
                            ...
                            ">
                <a href="<?php if(!$hasConnections) { echo 'delete/deletePaket.php?id=' . $data['id']; } ?>" class="px-5 py-2.5<?php if($hasConnections) { echo "disabled aria-disabled='true' cursor-not-allowed w-full h-auto";} ?>">Delete</a>
                         </button>
                        </td>
                        </tr>
